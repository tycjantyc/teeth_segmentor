...
class AugmentationFactory():
    ...
    def create(self, img_np, mask_np):
        ...
        subject = tio.Subject(
            image=tio.ScalarImage(tensor=torch.from_numpy(img_np).permute(3, 0, 1, 2)),
        ...
        img_aug = transformed.image.tensor.permute(1, 2, 3, 0).numpy().squeeze().astype(np.float32)
        mask_aug = transformed.mask.tensor.permute(1, 2, 3, 0).numpy().squeeze().astype(np.float32)
